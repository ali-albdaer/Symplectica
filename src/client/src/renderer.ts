...
class BodyMesh {
    ...
    applyRealisticTextures(enabled: boolean): void {
        ...
        if (textureName) {
            const url = `${import.meta.env.BASE_URL}textures/planets/${textureName}`;
        ...
        if (name === 'earth') {
            ...
            const cloudUrl = `${import.meta.env.BASE_URL}textures/planets/2k_earth_clouds.jpg`;
        ...
        if (name === 'saturn' && this.ringMesh && this.ringMesh.material instanceof THREE.ShaderMaterial) {
            const ringMat = this.ringMesh.material;
            const ringUrl = `${import.meta.env.BASE_URL}textures/planets/2k_saturn_ring_alpha.png`;
