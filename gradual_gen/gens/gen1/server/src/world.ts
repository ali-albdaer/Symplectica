...
export class WorldManager {
  ...
  serialize(): SerializedWorldState {
    ...
    const players: SerializedPlayer[] = this.getAllPlayers().map(p => ({
      ...
      position: [p.worldPosition.x, p.worldPosition.y, p.worldPosition.z] as [number, number, number],
      velocity: [p.worldVelocity.x, p.worldVelocity.y, p.worldVelocity.z] as [number, number, number],
      rotation: p.rotation,
  ...
  static deserialize(data: SerializedWorldState): WorldManager {
    ...
    for (const p of data.players) {
      const player: Player = {
        ...
        name: p.name,
        currentBodyId: p.currentBodyId,
        localPosition: Vec3.mutableVec3(p.position[0], p.position[1], p.position[2]),
        localVelocity: Vec3.mutableVec3(p.velocity[0], p.velocity[1], p.velocity[2]),
        worldPosition: Vec3.mutableVec3(p.position[0], p.position[1], p.position[2]),
        worldVelocity: Vec3.mutableVec3(p.velocity[0], p.velocity[1], p.velocity[2]),
        rotation: p.rotation,
        inVehicle: p.inVehicle,
        vehicleId: p.vehicleId,
        lastUpdate: Date.now(),
