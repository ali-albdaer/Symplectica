...
export class InputManager {
  ...
  toPlayerInputState(sequence: number): PlayerInputState {
    return {
      ...
      moveForward: this.state.forward,
      moveBackward: this.state.backward,
      moveLeft: this.state.left,
      moveRight: this.state.right,
      moveUp: this.state.up,
      moveDown: this.state.down,
      boost: this.state.boost,
      pitch: this.state.mouseDeltaY,
      yaw: this.state.mouseDeltaX,
      roll: 0,
    };
  }
