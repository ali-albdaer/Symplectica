...
(function (PlayerMode) {
    ...
    PlayerMode["SURFACE"] = "surface";
    /** Flying in space (EVA or spacecraft) */
    PlayerMode["SPACE"] = "space";
    /** World builder mode (orthographic view) */
    PlayerMode["WORLD_BUILDER"] = "world_builder";
...
export class Player {
    ...
    health = 100;
    isAlive = true;
    constructor(id, name, position = Vector3.zero(), config = {}) {
        this.id = id;
        this.name = name;
    ...
    applySpaceMovement(input, dt) {
        ...
        if (input.forward)
            thrustDir.add(forward);
        if (input.backward)
            thrustDir.sub(forward);
        if (input.right)
            thrustDir.add(right);
        if (input.left)
            thrustDir.sub(right);
     ...
     */
    spawnOnBody(body, latitude = 0, longitude = 0) {
        // Calculate surface position
        const cosLat = Math.cos(latitude);
        const sinLat = Math.sin(latitude);
        const cosLon = Math.cos(longitude);
        const sinLon = Math.sin(longitude);
        const surfaceDir = new Vector3(cosLat * cosLon, sinLat, cosLat * sinLon);
        const playerHeight = 2; // 2 meters above surface
        this.position = Vector3.add(body.position, Vector3.multiplyScalar(surfaceDir, body.radius + playerHeight));
    ...
    reconcile(serverState) {
        ...
        this.orientation.set(serverState.orientation[0], serverState.orientation[1], serverState.orientation[2], serverState.orientation[3]);
        this.health = serverState.health;
        // Replay unprocessed inputs
        const unprocessed = this.inputBuffer.filter(b => b.input.sequence > serverState.lastProcessedInput);
    ...
    applyNetworkState(state) {
        ...
        this.orientation.set(state.orientation[0], state.orientation[1], state.orientation[2], state.orientation[3]);
