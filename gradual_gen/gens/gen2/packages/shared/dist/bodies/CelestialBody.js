...
export class CelestialBody {
     ...
     * a = GM / (r² + ε²)
     */
    getGravitationalAcceleration(distance) {
     ...
     */
    getGravitationalForceOn(other) {
        const direction = Vector3.sub(this.position, other.position);
        const distSq = direction.lengthSquared();
        const dist = Math.sqrt(distSq);
        if (dist < 1e-10) {
            throw new Error(`CelestialBody.getGravitationalForceOn: Bodies ${this.id} and ${other.id} are at the same position (collision/singularity)`);
        }
     ...
     */
    updateSOI(parentMass, orbitRadius) {
     ...
     */
    getSurfaceVelocity(surfacePosition) {
