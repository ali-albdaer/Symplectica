 ...
 */
export function circularOrbitalVelocity(radius: number, centralMass: number): number {
  if (radius <= 0) {
 ...
 */
export function escapeVelocity(radius: number, mass: number): number {
  if (radius <= 0) {
 ...
 */
export function gravitationalAcceleration(distance: number, mass: number): number {
  if (distance <= 0) {
 ...
 */
export function specificAngularMomentum(position: Vector3, velocity: Vector3): Vector3 {
 ...
 */
export function eccentricityVector(
  position: Vector3,
  velocity: Vector3,
  centralMass: number
): Vector3 {
 ...
 */
export function stateVectorsToElements(
  position: Vector3,
  velocity: Vector3,
  centralMass: number
): OrbitalElements {
 ...
 */
export function visVivaVelocity(
  radius: number,
  semiMajorAxis: number,
  centralMass: number
): number {
 ...
 */
export function timeSincePeriapsis(
  trueAnomaly: number,
  eccentricity: number,
  period: number
): number {
...
export function hohmannTransfer(
  ...
  r2: number,
  centralMass: number
): { deltaV1: number; deltaV2: number; transferTime: number } {
  const mu = G * centralMass;
